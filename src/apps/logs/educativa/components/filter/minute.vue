...
const debug = Debug('apps:logs:educativa:components:filter:minute')
...
export default {
  ...
  _logsEducativaFilterMinute_components: {
    range: {
      source: {
      ...
      Array.each(pipelines, function (Pipeline) {
        ...
        if (!create_id || create_id === undefined || create_id === pipeline_id) {
          ...
          Array.each(template.input[0].poll.conn, function (conn, index) {
            template.input[0].poll.conn[index].requests = this.__components_sources_to_requests(this.$options._logsEducativaFilterMinute_components, pipeline_id)
          }.bind(this))
