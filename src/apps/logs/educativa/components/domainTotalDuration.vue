<template>
  <chart-tabular
    ...
    :always_update="false"
    :ref="type+'.per_domain_total'+ ((sum === true) ? '_sum' : '')"
    :id="type+'.per_domain_total' + ((sum === true) ? '_sum' : '')"
    :key="$route.path +'.'+ JSON.stringify($route.query)+'.'+type+'.per_domain_total'+ ((sum === true) ? '_sum' : '')"
    :stat="{
      data: [
        top_per_domain
      ],
...
const debug = Debug('apps:logs:educativa:components:domainTotalDuration')
...
export default {
  ...
  data () {
    return {
      id: 'logs.educativa.domainTotalDuration',
