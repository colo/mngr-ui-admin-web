...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.logs.educativa.filter.periodical.' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  ...
  filters: [
    ...
    // function (doc, opts, next, pipeline) {
    //   debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.logs.educativa.filter.periodical').conn_pollers)
    //
    //   let timeout
    //
    //   let _merge = function () {
    ...
    //   if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.logs.educativa.filter.periodical').conn_pollers)) { buffer.push(Object.clone(doc)) }
    //
    //   if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.logs.educativa.filter.periodical').conn_pollers)) { // || buffer_expire < Date.now()
    //     _merge()
    //   }
    //   // else {
