...
const debug = Debug('apps:logs:qmail:pipelines:filter:minute')
...
Array.each(IO(), function (io, index) {
  ios.push({
    id: 'input.logs.qmail.filter.minute.' + index,
    module: InputIO,
    index: index
  },)
})
...
export default {
  input: [
    {
      poll: {
        suspended: true,
        id: 'input.logs.qmail.filter.minute',
        conn: ios,
        // conn: [
        //
        //   Object.merge(
  ...
  filters: [
    ...
    function (doc, opts, next, pipeline) {
      debug('MERGE %o %o', doc, buffer, pipeline.get_input_by_id('input.logs.qmail.filter.minute').conn_pollers)
      ...
      if (buffer.length < Object.getLength(pipeline.get_input_by_id('input.logs.qmail.filter.minute').conn_pollers)) { buffer.push(Object.clone(doc)) }

      if (buffer.length >= Object.getLength(pipeline.get_input_by_id('input.logs.qmail.filter.minute').conn_pollers)) { // || buffer_expire < Date.now()
        _merge()
      }
      // else {
  ...
  output: [
    ...
    //
    //   if (!payload.err) { EventBus.$emit('input.logs.qmail.filter.minute.' + payload.metadata.input, payload) }
    //
    //   // if (!payload.err) { EventBus.$emit('log', payload) }
    // }
    function (payload) {
      if (!payload.err && /^input\.logs\.qmail\.filter\.minute\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.logs.qmail.filter.minute[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.logs.qmail.filter.minute.' + payload.metadata.input, payload)
      }
