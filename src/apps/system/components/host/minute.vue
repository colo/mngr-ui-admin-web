            ...
            <q-card-section>
              <template v-for="(name) in minute.plugins">
                ...
                <system-plugin-dygraph
                  ...
                  :stat="{
                    data: [],
                    length: $options[id].minute.length,
                    range: undefined,
                  }"
                  :dygraph="$options[id].minute.dygraph"
                  :interval="undefined"
                />
              </template>
            </q-card-section>
...
const debug = Debug('apps:system:components:host:minute')
...
export default {
  ...
  methods: {
    set_plugin_data: function (plugin, data, type, refresh) {
      ...
      if (this.$refs[plugin + '.' + type] && this.$refs[plugin + '.' + type][0] && !this.$refs[plugin + '.' + type][0].$options.plugin_data['os.' + plugin + '.' + type]) {
    ...
    **/
    ...
    destroy: function () {
      this.$options[this.id]['minute'].plugins_data = {}
    },
