...
const debug = Debug("apps:system:pipelines:input:io")
...
export default new Class({
  ...
  system: function(socket, next, doc){
    ...
    // }
    // else{
    //   debug('OTHERS os %o', doc)
    // }

    this.fireEvent('onDoc', [doc, { input_type: this, app: null }])

    // store.commit('system/clear')
    // store.commit('system/set', doc[type])
  },
  ...
  //
	// 	// arguments[1]()
	// 	// this.io.to('os').emit('response', 'a new user has joined the room saying '+arguments[2]);
	// 	// next(socket)
	// },
