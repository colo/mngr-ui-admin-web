...
export default {
  ...
  output: [
    function (payload) {
      if (!payload.err && /^input\.system\[.*\]$/.test(payload.id)) {
        payload.id = payload.id.replace('input.system[', '').slice(0, -1)
        debug('OUTPUT', payload)
        EventBus.$emit('input.system.' + payload.metadata.input, payload)
      }
