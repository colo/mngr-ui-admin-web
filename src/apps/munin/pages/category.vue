<template>
  <div>
    <template v-for="(host) in plugins_hosts">
      ...
      <q-card :key="host+'.'+category" v-if="plugins_config[host]">
        <q-card-section>
...
const debug = Debug('apps:munin:pages:category')
...
import Pipeline from '@apps/munin/pipelines/category'

import { requests, store } from '@apps/munin/sources/category/index'
...
export default {
  ...
  methods: {
    ...
    **/
    create_pipelines: function (next) {
      ...
      if (this.$options.pipelines['input.munin.category'] && this.$options.pipelines['input.munin.category'].get_input_by_id('input.munin.category')) {
        // let requests = this.__components_sources_to_requests(this.components)
        // if (requests.once) {
        //   this.$options.pipelines['input.munin.category'].get_input_by_id('input.munin.category').conn_pollers[0].options.requests.once.combine(requests.once)
        //   this.$options.pipelines['input.munin.category'].get_input_by_id('input.munin.category').conn_pollers[0].fireEvent('onOnceRequestsUpdated')
        // }
        //
        // if (requests.periodical) {
        //   this.$options.pipelines['input.munin.category'].get_input_by_id('input.munin.category').conn_pollers[0].options.requests.periodical.combine(requests.periodical)
        //   this.$options.pipelines['input.munin.category'].get_input_by_id('input.munin.category').conn_pollers[0].fireEvent('onPeriodicalRequestsUpdated')
        // }
      } else {
        let template = Object.clone(Pipeline)

        let pipeline_id = template.input[0].poll.id
        // let pipeline_id = 'input.munin.category'
  ...
  // },
  // mounted: function () {
  //   this.pipeline_id = 'input.munin.category'
  // },
  // create: function () {
