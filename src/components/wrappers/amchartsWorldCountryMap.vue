...
export default {
  ...
  methods: {
    ...
    handle_data: function (data) {
      debug('handle_data %o', data, this.$options['charts'][this.id].chart.series)
      // if (data && data !== null) { // && (newData.length > 0 || Object.getLength(newData) > 0)
      let self = this
      // if (self.$options['charts'][self.id].colorSet) {
      data = JSON.parse(JSON.stringify(data))
      let _data = []
      Array.each(data, function (country) {
        if (country !== undefined && country !== null) {
          let id = country.id
          // country.color = self.$options['charts'][self.id].colorSet.next()
          if (!country.color && self.followColorScheme === true) {
          ...
          if (id === undefined || id === null) { // get country ISO with name
            for (let id in countries) {
              if (countries.hasOwnProperty(id)) {
                let _country = countries[id]
                if (_country.country === country.name) { country.id = id }
              }
            }
          }
          ...
          // self.$options['charts'][self.id].chart.series[0].getPolygonById(id).fill = country.color
          // // else{
          // //   country.color = country.color
          // // }
          //
          // // debug('countries %o', country, { 'color': self.$options['charts'][self.id].colorSet.next() })
          // if (countries.hasOwnProperty(id)) {
          //   let _country = countries[id]
          //   if (_country.maps.length) {
          //     data.push({
          //       id: id,
          //       color: country.color,
          //       map: _country.maps[0]
      ...
      if (_data.length > 0) {
        ...
        this.$options['charts'][this.id].worldSeries.data = _data
      }

      debug('handle_data countries %o', _data)
      // }
      // }
    },
    init_chart: function () {
      ...
      this.$options['charts'][this.id].worldSeries = worldSeries
