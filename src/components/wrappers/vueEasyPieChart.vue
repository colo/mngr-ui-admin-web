...
<!-- {{chart.params.value}} -->
  ...
  <span class="easyPieChartLabel" style="font-size: 20px; top: 63px;">{{format_value(value || chart.params.value || percent || chart.params.percent )}}</span>
  <span class="easyPieChartUnits" style="font-size: 10px; top: 97px;">{{format_value(max)}} {{params.unit}}</span>

    <vue-easy-pie-chart
      v-bind="Object.merge(params, chart.params)"
      :percent="percent || chart.params.percent || value || chart.params.value "
      :id="id"
      :ref="id"
      v-on:step="step"
    />
