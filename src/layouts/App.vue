    ...
    content-class="bg-white" -->
      <q-scroll-area class="fit">
        <q-list padding class="text-grey-8">
          ...
          <q-item class="GNL__drawer-item" v-ripple v-for="link in links2" :key="link.text" clickable :to="link.to">
            <q-item-section avatar>
              <q-icon :name="link.icon" />
            </q-item-section>
            <q-item-section>
...
export default {
  ...
  data () {
    return {
      ...
      links2: [
        { icon: 'search', text: 'Checks', to: {name: 'checks'}},
        { icon: 'search', text: 'Alerts', to: {name: 'alerts'}}
